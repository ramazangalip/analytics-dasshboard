...
const Badge = ({percentage} :{percentage:number}) =>{
    ...
    return (
        <span className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${isPositive? positiveClass : isNeutral ? neutralClass : negativeClass}`}>
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="grid w-full mx-auto grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <Card className="w-full mx-auto max-w-xs">
                    <p className="text-tremor-default text-dark-tremor-content">Visitors today
            ...
            <Card className="flex flex-col sm:grid grid-cols-4 gap-6">
                <h2 className="w-full text-dark-tremor-content-strong text-center sm:left-left font-semibold text-xl">Bu haftaki ziyaretçi:</h2>
                <div className="col-span-3 flex items-center justify-between flex-wrap gap-8">
                    {topCountries?.map(([countryCode, number], index) => (
                        <div key={index} className="flex items-center gap-3 text-dark-tremor-content-strong">
                            <p>{countryCode}</p>
